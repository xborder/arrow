...
class PollInfoConformanceTest : public ::testing::Test {
 protected:
  ...
  void Connect(bool use_poll_info = true) {
    ...
    ASSERT_EQ(SQL_SUCCESS, SQLAllocHandle(SQL_HANDLE_DBC, environment_, &connection_));
    const std::string connection_string =
        "DRIVER={Apache Arrow Flight SQL ODBC Driver};HOST=127.0.0.1;PORT=" +
        std::to_string(port_) + ";useEncryption=false;UseWideChar=false;UsePollInfo=" +
        (use_poll_info ? "true;" : "false;");
    auto input = Wide(connection_string);
    SQLWCHAR output[2048] = {};
    SQLSMALLINT output_length = 0;
    ASSERT_EQ(SQL_SUCCESS,
              SQLDriverConnectW(connection_, nullptr, input.data(), SQL_NTS, output, 2048,
...
TEST_F(PollInfoConformanceTest, MultiStepProgressiveFetch) {
  Connect();
...
TEST_F(PollInfoConformanceTest, PreparedBindsOnce) {
  ...
  const SQLRETURN bind_status =
      SQLBindParameter(statement_, 1, SQL_PARAM_INPUT, SQL_C_SBIGINT, SQL_BIGINT, 0, 0,
  ...
  const SQLRETURN diagnostic_status =
      SQLGetDiagRecW(SQL_HANDLE_STMT, statement_, 1, sql_state, &native_error, message,
                     512, &message_length);
  ASSERT_TRUE(IsSuccess(bind_status))
      << "SQLBindParameter returned " << bind_status
      << "; diagnostic_status=" << diagnostic_status
      << "; SQLSTATE=" << Narrow(sql_state, 5) << "; native_error=" << native_error
      << "; message="
...
TEST_F(PollInfoConformanceTest, MetadataCatalogs) {
  ...
  auto catalogs = Wide("%");
  auto empty = Wide("");
  ASSERT_TRUE(IsSuccess(SQLTablesW(statement_, catalogs.data(), SQL_NTS, empty.data(),
                                   SQL_NTS, empty.data(), SQL_NTS, nullptr, 0)));
...
TEST_F(PollInfoConformanceTest, LateErrorSurfacesAtNextFetch) {
  Connect();
  ASSERT_TRUE(IsSuccess(ExecuteDirect("late-error")));
  ASSERT_TRUE(IsSuccess(SQLFetch(statement_)));
  SQLBIGINT value = 0;
  SQLLEN indicator = 0;
  ASSERT_TRUE(IsSuccess(
      SQLGetData(statement_, 2, SQL_C_SBIGINT, &value, sizeof(value), &indicator)));
  EXPECT_EQ(1, value);
...
TEST_F(PollInfoConformanceTest, QueryTimeoutInterruptsBlockedPoll) {
  Connect();
  ASSERT_TRUE(IsSuccess(
      SQLSetStmtAttr(statement_, SQL_ATTR_QUERY_TIMEOUT,
...
TEST_F(PollInfoConformanceTest, SQLCancelInterruptsBeforeResultSet) {
  ...
  const auto start = std::chrono::steady_clock::now();
  std::thread execute([&] { execute_status = ExecuteDirect("blocked-poll"); });
...
TEST_F(PollInfoConformanceTest, SQLCancelInterruptsBlockingFetch) {
  Connect();
  ASSERT_TRUE(IsSuccess(ExecuteDirect("cancel-observable")));
  ASSERT_TRUE(IsSuccess(SQLFetch(statement_)));
  ASSERT_TRUE(IsSuccess(SQLFetch(statement_)));

  SQLRETURN fetch_status = SQL_SUCCESS;
  std::thread fetch([&] { fetch_status = SQLFetch(statement_); });
  std::this_thread::sleep_for(std::chrono::milliseconds(250));
  const auto start = std::chrono::steady_clock::now();
  EXPECT_TRUE(IsSuccess(SQLCancel(statement_)));
  fetch.join();
  const auto elapsed = std::chrono::steady_clock::now() - start;
  EXPECT_EQ(SQL_ERROR, fetch_status);
  EXPECT_LT(elapsed, std::chrono::seconds(3));
}

TEST_F(PollInfoConformanceTest, CloseIncompleteResultAttemptsCancellation) {
  Connect();
  ASSERT_TRUE(IsSuccess(ExecuteDirect("cancel-observable")));
  ASSERT_TRUE(IsSuccess(SQLFetch(statement_)));
  EXPECT_TRUE(IsSuccess(SQLCloseCursor(statement_)));
}
